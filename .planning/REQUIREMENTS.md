...
- [x] **UX-01**: 已绑定且有活跃会话的话题中，用户直接输入文字即视为聊天提示，无需 `/oc chat` 前缀
- [x] **UX-02**: 异步任务路径返回 null 而非空字符串，不再出现空消息气泡
- [x] **UX-03**: 机器人回复中包含当前绑定状态指示（项目名/会话ID），用户随时知道"我在哪"

### 命令与路由 (Commands & Routing)

- [x] **CMD-01**: 重新设计命令集，匹配手动控制流（项目选择 → 会话选择/创建 → 绑定 → 对话）
- [x] **CMD-02**: 每种状态下只显示/允许有效的命令，白名单包含所有别名
- [x] **CMD-03**: 所有错误消息包含下一步操作建议，不只是报错原因
- [x] **CMD-04**: 每个操作完成后，系统性地提示用户下一步该做什么
...
*Last updated: 2026-04-10 — Phase 2 requirements complete (UX-01, UX-02, UX-03, CMD-01, CMD-02, CMD-03, CMD-04, COMPAT-02)*
