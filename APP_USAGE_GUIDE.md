...
| 话题状态 | 说明 | 判断条件 |
|---------|------|---------|
| **话题外** | 未在任何话题中 | topicId 为 null 或空 |
| **话题未初始化** | 话题存在但未绑定会话 | topicId 存在，无 sessionId |
| **话题已初始化** | 话题已绑定会话 | topicId 和 sessionId 都存在 |

#### 命令权限矩阵

| 命令 | 话题外 | 话题未初始化 | 话题已初始化 | 说明 |
|------|-------|------------|------------|------|
...
无需任何配置，使用 `chatnow` 命令立即开始：

```
/opencode chatnow 帮我写个排序函数
```
...
**系统自动完成**：
- 在 `项目启动目录/workspace/{YYYY-MM-DD}/` 创建新会话
  - 例如：`/root/workspace/feishu-backend/workspace/2026-02-07/`
- 自动绑定到当前话题
- 返回会话基础信息 + 对话结果
...
#### 5.2 向导引导流程（首次使用，Phase 3）

当首次进入未绑定的话题时，系统自动弹出**3步交互式向导卡片**，帮助完成会话绑定：

```
Step 1 — 选择项目（自动弹出）:
  机器人: 🎯 欢迎使用 OpenCode！第 1 步：选择项目
    [按钮] feishu-backend   [按钮] other-project   [取消]

Step 2 — 点击项目后，选择会话:
  机器人: 📋 第 2 步：选择会话
    [按钮] 重构登录 — 2小时前   [按钮] 添加测试 — 1天前   [新建会话]

Step 3 — 确认绑定:
  机器人: ✅ 确认绑定 ses_abc123 到当前话题？
    [确认]   [取消]
```

**注意**：
- 向导进行中，非向导命令会被拦截，需先完成或取消向导
- 取消向导：点击"取消"按钮，或输入 `/oc wizard_cancel`
- 向导 10 分钟后自动超时
...
/opencode chatnow <内容>
```
- 别名：`/opencode cn <内容>` 或 `/oc cn <内容>`
- 用途：自动创建会话并绑定到话题，立即开始对话
...
机器人: 📋 [交互式卡片] feishu-backend 的最近会话
  [按钮] 重构登录模块 — 2小时前   [按钮] 添加单元测试 — 1天前
  [按钮] + 新建会话

💡 在话题中执行 sessions 命令会优先显示**交互式卡片**，
   点击会话按钮可直接绑定，无需手动输入会话 ID。
   若卡片发送失败，自动降级为纯文本格式。
...
1. **直接对话**：在已初始化的话题中，无需 `/opencode chat` 前缀，直接输入文字即可
...
**状态指示器**：每条 OpenCode 回复头部自动显示当前绑定状态：
```
📎 opencode | ses_abc123

[回复内容...]
```
- 已绑定会话：`📎 opencode | ses_abc123`
- 未绑定会话：`📎 opencode | 未绑定会话`
- 非 OpenCode 应用回复不显示状态行
- `help` 命令回复不显示状态行
...
```
- `projects` → 建议查看 sessions
- `sessions` → 建议绑定 sc
- `sc` → 建议直接输入问题
- `chat`/`chatnow` → 不附加建议（已在对话中）

**直接输入路由**：在已初始化话题中，输入纯文本（不以 `/` 开头）自动合成为 `/opencode chat <文字>` 并路由到 OpenCode。未初始化话题中的纯文本会显示初始化引导。
...
**Q4: OpenCode 需要初始化吗？**
```
A: 首次进入话题时会自动弹出向导卡片引导完成绑定（Phase 3 新增）。
   也可以手动执行：/oc sessions <项目> 查看会话，/oc sc <ID> 绑定。
```

**Q6: 会话列表是卡片还是文本？**
```
A: 在话题中执行 /oc sessions 优先显示交互式卡片（可点击绑定），
   卡片失败或非话题环境时自动降级为纯文本格式。
```
